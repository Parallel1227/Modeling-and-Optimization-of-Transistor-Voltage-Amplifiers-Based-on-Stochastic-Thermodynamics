...
for V in np.arange(0,V_DD_max,0.1):
	Vin[i] = V
	J_d1[i], V_D1[i] = PMOS(15.0, Vin[i], -2.0)
	J_d2[i], V_D2[i] = PMOS(15.0, Vin[i], -1.0)
	J_d3[i], V_D3[i] = PMOS(15.0, Vin[i], 0.0)
	J_d4[i], V_D4[i] = PMOS(15.0, Vin[i], 1.0)
	J_d5[i], V_D5[i] = PMOS(15.0, Vin[i], 2.0)
	i = i + 1
	
with open('/results/PMOS_transfer.txt', 'w') as f:
