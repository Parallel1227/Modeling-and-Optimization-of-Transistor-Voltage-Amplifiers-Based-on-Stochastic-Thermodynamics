...
for V in np.arange(0,V_DD_max,0.1):
	V_DD[i] = V
	J_d1[i], V_D1[i] = NMOS(V_DD[i], -5.0, 0.0)
	J_d2[i], V_D2[i] = NMOS(V_DD[i], -3.0, 0.0)
	J_d3[i], V_D3[i] = NMOS(V_DD[i], -1.0, 0.0)
	J_d4[i], V_D4[i] = NMOS(V_DD[i], 0.0, 0.0)
	J_d5[i], V_D5[i] = NMOS(V_DD[i], 1.0, 0.0)
	i = i + 1

with open('/results/NMOS_output.txt', 'w') as f:
